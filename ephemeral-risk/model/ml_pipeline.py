...
from model.correlator import group_anomalies_into_incidents
from model.detector import score_with_global_model, get_score_distribution
from model.features import FEATURE_COLS, MODEL_FEATURE_COLS, calculate_features_from_events
...
class _LiveMLPipeline:
    ...
    def _blocked_principal_in(incident: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if not principals:
            return None
        try:
            from backend.database import is_principal_blocklisted
            ...
            if is_anomaly:
                try:
                    ...
                    if not anomalies_df.empty:
                        ...
                        # Map risk_score to severity so the correlator groups them with the correct severity
                        def _map_risk_to_severity(score: float) -> str:
                            if score >= 80: return "CRITICAL"
                            if score >= 60: return "HIGH"
                            if score >= 30: return "MEDIUM"
                            return "LOW"
                        anomalies_df["severity"] = anomalies_df["risk_score"].apply(_map_risk_to_severity)
