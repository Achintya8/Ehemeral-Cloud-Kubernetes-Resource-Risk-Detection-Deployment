...
from backend.auth import authenticate_user, create_access_token, decode_token, get_current_user, require_admin, hash_password
...
from model.features import calculate_features_from_events
from model import detector
from model.ml_pipeline import get_pipeline_stats, process_event, seed_pipeline_events
from model.narrative import build_incident_report
...
class CreateUserRequest(StrictRequestModel):
    username: StrictStr = Field(min_length=3, max_length=64, pattern=r"^[A-Za-z0-9._-]+$")
    password: StrictStr = Field(min_length=8, max_length=128)
    role: StrictStr = Field(pattern=r"^(admin|analyst)$")
...
def _cleanup_false_positive_incidents() -> None:
    ...
    """
    from backend.database import get_connection, _DB_LOCK
        ...
        try:
            cursor = connection.execute(
                ...
                WHERE report_text LIKE '%0 correlated ephemeral resource(s)%'
                   ...
                   OR node_count = 0
                   OR incident_id = '4c549893-f874-446d-b6bc-da62de697a97'
                """
            )
            removed = cursor.rowcount
            
            # Also resolve any stale incidents that have user feedback but are still marked active
            cursor_resolve = connection.execute(
                """
                UPDATE incidents
                SET status = 'resolved'
                WHERE user_feedback IS NOT NULL AND status = 'active'
                """
            )
            resolved_count = cursor_resolve.rowcount
    ...
    if removed:
        print(f"  [cleanup] Removed {removed} false-positive / malformed / stale incident(s).")
    if resolved_count:
        print(f"  [cleanup] Auto-resolved {resolved_count} stale incident(s) with pre-existing user feedback.")
...
async def _auto_remediate_if_critical(record: dict) -> None:
    ...
    async def _run_remediation():
        from backend import remediation as _rem
        try:
            # 1. Isolate the pod
...
async def k8s_queue_processor():
    from model.ml_pipeline import process_event
    ...
    while True:
        try:
            ...
            if analysis.cluster is not None:
                ...
                await asyncio.to_thread(dispatch_alert, incident_record)
...
async def startup_event() -> None:
    ...
    if False:  # Disabled live watcher to prevent processing of cluster events
...
@app.get("/api/users")
@limiter.limit("60/minute")
async def api_list_users(request: Request, _: Dict[str, Any] = Depends(require_admin)) -> JSONResponse:
    users = fetch_users()
...
async def api_create_user(
    ...
    user_id = create_user(payload.username, hashed, payload.role)
    return JSONResponse({
        "status": "success",
        "user": {
            "id": user_id,
...
async def api_delete_user(
    ...
    # Do not allow admin to self-delete
    if target_user["username"] == current_user["username"]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Self-deletion of your own administrator account is prohibited",
        )
    ...
    return JSONResponse({
        "status": "success",
        "message": f"Successfully deleted user account '{target_user['username']}'"
    })
...
async def incident_feedback(
    ...
    """Submit user approval or disapproval feedback for an AI triage analysis or automated response."""
    if not incident_id or len(incident_id) > 200:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid incident_id")

    from backend import database
    inc = database.fetch_incident_with_events(incident_id)
    if not inc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Incident {incident_id} not found")

    database.save_incident_feedback(incident_id, payload.feedback)

    # Resolve target resource info
    target_res = "unknown"
    target_ns = "default"
    principal_id = None
    if inc.get("events"):
        target_res = next((e.get("resource_name") for e in inc["events"] if e.get("resource_name")), "unknown")
        target_ns = next((e.get("namespace") for e in inc["events"] if e.get("namespace")), "default")
        principal_id = next((e.get("principal_id") for e in inc["events"] if e.get("principal_id")), None)

    # 1. Log Remediation Approval
    if payload.feedback.startswith("Remediation Approved"):
        database.insert_action_log(
            incident_id=incident_id,
            action_type="approve_containment",
            target_resource=target_res,
            namespace=target_ns,
            source_ip=inc.get("pivot_ip") or "",
            principal_id=principal_id or "",
            operator=current_user.get("username", "system"),
            result="success",
            message="Auto-remediation containment approved by analyst.",
        )
        if hasattr(app.state, "broadcaster"):
            await app.state.broadcaster.publish(
                "action_log", {"entries": database.fetch_action_log(50)}
    ...
    # 2. Log Remediation Disapproval / Rollback
    elif payload.feedback.startswith("Remediation Disapproved"):
        if principal_id:
            database.release_blocklist_entry(principal_id)

        database.insert_action_log(
            incident_id=incident_id,
            action_type="rollback_containment",
            target_resource=target_res,
            namespace=target_ns,
            source_ip=inc.get("pivot_ip") or "",
            principal_id=principal_id or "",
            operator=current_user.get("username", "system"),
            result="success",
            message=f"Auto-remediation containment rolled back due to analyst disapproval. Notes: {payload.feedback.replace('Remediation Disapproved: ', '')}",
        )
        ...
        if hasattr(app.state, "broadcaster"):
            ...
            await app.state.broadcaster.publish(
                "blocklist", {"entries": database.fetch_active_blocklist()}
            )

    # Resolve the incident in the database and broadcast remediation event so it is filtered out of active lists
    database.resolve_incident(incident_id)
    ...
    if hasattr(app.state, "broadcaster"):
        await app.state.broadcaster.publish(
            ...
            {
                "incident_id": incident_id,
                "action_type": "containment_feedback",
                "operator":    current_user.get("username", "system"),
                "timestamp":   datetime.now(timezone.utc).isoformat(timespec="seconds"),
                "status":      "success",
                "message":     "Analyst resolved incident via containment verification.",
            },
        )
...
def generate_summary_report(
    ...
    try:
        # 1. Query the database based on the timeframe
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        
        # Calculate time threshold
        time_threshold = (datetime.now(timezone.utc) - timedelta(hours=req.timeframe_hours)).isoformat()
        
        # Get Total Events
        cursor.execute("SELECT COUNT(*) FROM events WHERE timestamp >= ?", (time_threshold,))
        total_events = cursor.fetchone()[0]
        
        # Get Risky Events
        cursor.execute("SELECT COUNT(*) FROM events WHERE timestamp >= ? AND (severity = 'CRITICAL' OR risk_score >= 80)", (time_threshold,))
        risky_events = cursor.fetchone()[0]
        
        # Get Top 3 Critical Incidents for context
        cursor.execute("""
            SELECT event_id, event_name, principal_id, resource_name, risk_score 
            FROM events 
        ...
        risk_percentage = round((risky_events / total_events * 100), 1) if total_events > 0 else 0.0

        # Format incident context for the LLM
        incident_context = ""
        for idx, row in enumerate(critical_rows, 1):
            incident_context += f"{idx}. {row[1]} (INC-{row[0][:6]})\n   Resource: {row[3]}\n   Identity: {row[2]}\n   Risk Score: {row[4]}/100\n\n"

        # 2. Build the Strict Prompt
        current_date_str = datetime.now(timezone.utc).strftime("%B %d, %Y")
        system_prompt = """You are a Senior Cloud Security Analyst. Generate an executive threat summary strictly matching this exact format template. Do not deviate.

EPHEMERAL RISK SUMMARY — [Current Date]
=========================================
Total Ephemeral Resources ([Timeframe]): [Total Count]
Risky Resources Flagged: [Risky Count] ([Percentage]%)
Incidents Clustered: [Incident Count]

CRITICAL INCIDENTS
[List the incidents provided in the context below. Ensure you include Identity, Behavior, Risk Score, MITRE mappings, and Recommended Actions for each.]
""".replace("[Current Date]", current_date_str)
        
        user_prompt = f"Timeframe: Last {req.timeframe_hours} hours.\nTotal: {total_events}\nRisky: {risky_events} ({risk_percentage}%)\n\nIncident Context:\n{incident_context}"
        ...
        if response.status_code == 200:
            report_text = response.json().get("choices", [{}])[0].get("message", {}).get("content", "Error parsing LLM response.")
            return {"report_text": report_text}
        else:
            return {"report_text": f"Cloud LLM Error: {response.text}"}

    except Exception as e:
        return {"report_text": f"Internal Server Error: {str(e)}"}


@app.post("/api/incidents/{incident_id}/narrative")
def generate_incident_narrative(
    ...
    try:
        ...
        report_data = {}
        if incident.get("report_text"):
            try:
                report_data = json.loads(incident["report_text"])
            except Exception:
                pass

        evidence = report_data.get("correlated_evidence", {})
        who = evidence.get("who", "Unknown")
        what = evidence.get("what", "Unknown")
        when = evidence.get("when", "Unknown")
        where = evidence.get("where", "Unknown")
        why_risky = evidence.get("why_risky", "Unknown")
        scenario = report_data.get("scenario", "Unknown")
        mitre_tactics = report_data.get("mitre_tactics", [])
        intent_summary = report_data.get("intent_summary", "Unknown")

        events = incident.get("events", [])
        events_summary = ""
        for idx, e in enumerate(events[:10], 1):
            events_summary += f"- Event {idx}: {e.get('timestamp')} | {e.get('event_name')} | Severity: {e.get('severity')} | Actor: {e.get('principal_id')} | Resource: {e.get('resource_name')} | Risk: {e.get('risk_score')}\n"

        system_prompt = """You are a Principal Cloud Security Analyst.
Analyse the raw 'Correlated Events' telemetry to infer actionable security evidence, attacker intent, and compliance/tactic mappings.
CRITICAL: Do NOT just repeat the high-level 'Aggregated Evidence' (Who/What/When/Where). You must dig into the specific Correlated Events (e.g. timestamps, specific API verbs, burst counts) to formulate 3 to 5 detailed, highly technical insight bullet points.

Verify and generate mappings for the following frameworks:
1. MITRE ATT&CK:
   Verify if the following apply:
   - T1578 (Modify Cloud Compute Infrastructure): if compute workloads or configs are created/deleted/modified.
   - T1496 (Resource Hijacking): if compute resources are abused (e.g. crypto-mining, burst execution).
   - T1190 (Exploit Public-Facing Application): if initial vector is via ingress or public IP.
   Also include any applicable rule-based MITRE tactics (e.g. Valid Accounts (T1078.002)) provided below. Use the exact full strings.
2. NIST SP 800-53:
   Verify if the following apply:
   - CM-8 (Information System Component Inventory): if the event involves listing, mapping, or modifications of container/pod/resource inventory.
   - SI-4 (Information System Monitoring): if the event involves system anomaly/behavioral alarms.
   - IR-4 (Incident Handling): if containment or remediation actions are recommended/applied.
3. CIS Benchmarks:
   Verify if the following apply:
   - Kubernetes CIS Benchmark: if Kubernetes RBAC, service accounts, namespaces, or pod security configuration deviations are detected.
   - Cloud provider CIS Benchmarks: if cloud IAM, cloud metadata service, VPC flow anomalies, or token credentials are abused.

For any of the above, if the mapping applies, include it in the respective list. If none apply, return an empty list.

Produce a structured JSON triage report exactly matching this schema:
{
  "incident_id": "string",
  "severity": "string",
  "risk_score": integer,
...
Output ONLY valid JSON. Do not include markdown formatting blocks like ```json or any other conversational text."""

        user_prompt = f"""Incident ID: {incident_id}
Severity: {incident.get('severity')}
Risk Score: {incident.get('risk_score')}/100
Classified Scenario: {scenario}
...
- WHERE: {where}
- WHY RISKY: {why_risky}
- Rule-based Intent Summary: {intent_summary}
- Rule-based MITRE Tactics: {', '.join(mitre_tactics)}

Correlated Events (up to 10):
{events_summary}"""
        ...
        if response.status_code == 200:
            narrative = response.json().get("choices", [{}])[0].get("message", {}).get("content", "Error parsing LLM response.")
