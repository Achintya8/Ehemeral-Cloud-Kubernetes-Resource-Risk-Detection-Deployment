...
def normalize_telemetry_event(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    if log_type == "cloudtrail" or "eventSource" in raw or "userIdentity" in raw:
        return normalize_cloudtrail_event(raw)
    if log_type == "vpc_flow" or {"srcaddr", "dstaddr"}.issubset(raw.keys()) or {"src_addr", "dst_addr"}.issubset(raw.keys()):
        return normalize_vpc_flow_event(raw)
    if (
        log_type == "k8s_audit"
        or raw.get("apiVersion") == "audit.k8s.io/v1"
        or "objectRef" in raw
        or "pod_name" in raw
        or "namespace" in raw
    ):
